...
static NSURL* FindShaderResourceURL(NSString* name, NSString* ext, NSString* subDir)
{
    NSURL *url = [[NSBundle mainBundle] URLForResource:name withExtension:ext subdirectory:subDir];
    if (url)
    {
        return url;
    }
    ...
    {
        url = [bundle URLForResource:name withExtension:ext subdirectory:subDir];
        if (url)
        {
            return url;
        }
...
void CubismShader_Metal::GenerateShaders(id<MTLDevice> device)
{
    ...
    NSURL *libraryURL = FindShaderResourceURL(@"MetalShaders", @"metallib", subDir);
    id<MTLLibrary> shaderLib = libraryURL ? [device newLibraryWithURL:libraryURL error:nil] : nil;
    if(!shaderLib)
    {
        NSURL *sourceURL = FindShaderResourceURL(@"MetalShadersSource", @"txt", subDir);
        NSString *source = sourceURL ? [NSString stringWithContentsOfURL:sourceURL encoding:NSUTF8StringEncoding error:nil] : nil;
        if (source)
        {
            NSError *compileError = nil;
            shaderLib = [device newLibraryWithSource:source options:nil error:&compileError];
            if (!shaderLib && compileError)
            {
                NSLog(@" ERROR: Couldnt compile fallback Metal shader source: %@", compileError.localizedDescription);
            }
        }
    }
    if(!shaderLib)
...
CubismShader_Metal::ShaderProgram* CubismShader_Metal::LoadShaderProgramFromFile(const csmString& vertShaderFileName, const csmString& fragShaderFileName, const csmString& vertShaderSrc, const csmString& fragShaderSrc, id<MTLDevice> device)
{
    ...
    NSURL *vertShaderFileLibURL = FindShaderResourceURL(vertShaderFileNameStr, @"metallib", subDir);

    NSString *fragShaderFileNameStr = [NSString stringWithCString:fragShaderFileName.GetRawString() encoding:NSUTF8StringEncoding];
    NSURL *fragShaderFileLibURL = FindShaderResourceURL(fragShaderFileNameStr, @"metallib", subDir);

    if (vertShaderFileLibURL == nil || fragShaderFileLibURL == nil)
    {
        return nil;
    }
